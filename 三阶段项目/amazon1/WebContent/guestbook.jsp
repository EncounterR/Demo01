...
<div id="position" class="wrap">
	您现在的位置：<a href="${pageContext.request.contextPath}/index.jsp">亚马逊</a> &gt; 在线评论
</div>
			...
			<ul>
				...
				<li>
					<dl>
						...
						<dd class="author"><span>作者：${item.hc_nick_name }</span></dd>
						<dd>评论时间：${item.hc_create_time }</dd>
						<c:if test="${item.hc_reply ==''}">
						<dd>商家还未回复，请稍等</dd>
						</c:if>
						<c:if test="${item.hc_reply !=''}">
						<dd>商家回复：${item.hc_reply }</dd>
						<dd>回复时间：${item.hc_reply_time }</dd>
						</c:if>
					</dl>
				</li>
				</c:forEach>
