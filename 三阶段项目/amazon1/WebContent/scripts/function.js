...
function Checkexist() {
	...
	var flag=true
	$.ajax({
		url : "HwuaUserServlet?org=checkName",// 请求的servlet地址
		type : "POST",// 请求方式
		async: false,
...
function CheckNickname() {
	...
	var flag=true
	$.ajax({
		url : "HwuaUserServlet?org=CheckNickname",// 请求的servlet地址
		type : "POST",// 请求方式
		async: false,
		data : "nickName=" + nickName.value,// 发送到服务器的数据
		dataType : "text",// 设置返回数据类型
		success : function(test) {
			if (test == 1) {
				var msgBox = document.getElementById("nName");
				msgBox.style.display = "inline";
				msgBox.innerHTML = "昵称已存在！";
				flag= false;
			} else {
				var msgBox = document.getElementById("nName");
				msgBox.style.display = "inline";
...
function emailExist() {
	...
	$.ajax({
		url : "HwuaUserServlet?org=checkEmail",// 请求的servlet地址
		type : "POST",// 请求方式
...
function checkValidateCode() {
	...
	var flag=true;
	$.ajax({
		url : "CheckCodeServlet",// 请求的servlet地址
		type : "Post",// 请求方式
		async: false,
		data : "veryCode1=" + veryCode,// 发送到服务器的数据
		dataType : "text",// 设置返回数据类型
		success : function(test) {
			if (test != 1) {
